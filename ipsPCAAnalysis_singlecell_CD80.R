...
expData_InHouse <- read.table(file="/public/ZhangJinLab/backdata/lahdata/cdata/AABBDDCD80_gene_TPM.txt",sep="\t",header=TRUE,row.names=1,stringsAsFactors=FALSE)
...
expData <- expData[,c("AACD80","AANonCD80","BBCD80","BBNonCD80","DDCD80","DDNonCD80","IPSDM M1 rep1","IPSDM M1 rep2","IPSDM M1 rep3","IPSDM M2 rep1","IPSDM M2 rep6","IPSDM MAC rep2","IPSDM MAC rep3","IPSDM MAC rep4")]
expData <- expData[which(apply(expData,1,maxvalue) > 1),]
batch <- as.factor(rep(c("InHouse","SRP039361"), c(6,8)))
...
stageNames <- rep(c("AACD80","AANonCD80","BBCD80","BBNonCD80","DDCD80","DDNonCD80","IPSDM M1","IPSDM M2","IPSDM MAC"),c(1,1,1,1,1,1,3,2,3))
...
p <- ggplot(plotdata,aes(x=x,y=y,colour=factor(pointColor,levels=c("AACD80","AANonCD80","BBCD80","BBNonCD80","DDCD80","DDNonCD80","IPSDM M1","IPSDM M2","IPSDM MAC")),shape=factor(pointColor,levels=c("AACD80","AANonCD80","BBCD80","BBNonCD80","DDCD80","DDNonCD80","IPSDM M1","IPSDM M2","IPSDM MAC")))) + geom_point(size=3) + scale_shape_manual(values=c(1,2,3,4,5,6,7,8,9))
...
ggsave("/public/ZhangJinLab/backdata/lahdata/cdata/PCA_2DRes_PC1_PC2_CD80.pdf",width=4,height=3)
...
p <- p + theme(legend.title=element_blank())
p <- p + theme(panel.grid.major =element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(),axis.line = element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11))
ggsave("/public/ZhangJinLab/backdata/lahdata/cdata/PCA_2DRes_PC1_PC3_CD80.pdf",width=4,height=3)
...
p <- p + theme(legend.title=element_blank())
p <- p + theme(panel.grid.major =element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(),axis.line = element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11))
ggsave("/public/ZhangJinLab/backdata/lahdata/cdata/PCA_2DRes_PC2_PC3_CD80.pdf",width=4,height=3)

library(scatterplot3d)
x <- (max3PCs[,1]-min(max3PCs[,1]))/(max(max3PCs[,1])-min(max3PCs[,1]))
y <- (max3PCs[,2]-min(max3PCs[,2]))/(max(max3PCs[,2])-min(max3PCs[,2]))
z <- (max3PCs[,3]-min(max3PCs[,3]))/(max(max3PCs[,3])-min(max3PCs[,3]))
pdf(file="/public/ZhangJinLab/backdata/lahdata/cdata/PCA_3DRes_CD80.pdf",width=18,height=9)
par(xpd=TRUE,mgp=c(2,0.8,0),oma=c(0.2,0.2,0.2,0.2),lwd=3)
scatterplot3d(x,y,z,color=rep(rainbow(9),c(1,1,1,1,1,1,3,2,3)),pch=rep(seq(1,9),c(1,1,1,1,1,1,3,2,3)),xlab="PC1",ylab="PC2",zlab="PC3",mar=c(4,4,4,28),cex.axis=2,cex.lab=3,cex.symbols=2.5)
legend("topright",c("AACD80","AANonCD80","BBCD80","BBNonCD80","DDCD80","DDNonCD80","IPSDM M1","IPSDM M2","IPSDM MAC"),inset=c(-0.47,-0.06),pch=seq(1,6),col=rainbow(6),cex=2,y.intersp=1.0)
dev.off()
