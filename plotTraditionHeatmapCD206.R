...
dataMat <- cbind(dataMatAAMRC1[AABBDD],dataMatAANonMRC1[AABBDD],dataMatBBMRC1[AABBDD],dataMatBBNonMRC1[AABBDD],dataMatDDMRC1[AABBDD],dataMatDDNonMRC1[AABBDD])
sampInfo <- factor(c("AAMRC1","AANonMRC1","BBMRC1","BBNonMRC1","DDMRC1","DDNonMRC1"),levels=c("AAMRC1","AANonMRC1","BBMRC1","BBNonMRC1","DDMRC1","DDNonMRC1"))
...
plotdata_sc_clulabels <- rep(c("AAMRC1","AANonMRC1","BBMRC1","BBNonMRC1","DDMRC1","DDNonMRC1"),each=ncol(dataMat))
...
plotdata_sc$plotdata_sc_clulabels <- factor(plotdata_sc_clulabels,levels=c("HM_M1","IPS_M1_rep1","IPS_M1_rep2","IPS_M1_rep3","IPS_M1_rep4","IPS_M1_rep5","AAMRC1","AANonMRC1","BBMRC1","BBNonMRC1","DDMRC1","DDNonMRC1","HM_M2","IPS_M2_rep1","IPS_M2_rep2","IPS_M2_rep3","IPS_M2_rep4","IPS_M2_rep5"))
...
pdf(file="/public/ZhangJinLab/project_erv/Dpan/IPSAnaly/M1M2_traditional_cluster_CD206.pdf",width=10,height=10)
...
pdf(file="/public/ZhangJinLab/project_erv/Dpan/IPSAnaly/M1M2_traditional_nocluster_CD206.pdf",width=10,height=10)
...
dataMat <- cbind(dataMatAAMRC1[AABBDD],dataMatBBMRC1[AABBDD],dataMatDDMRC1[AABBDD])
sampInfo <- factor(c("AAMRC1","BBMRC1","DDMRC1"),levels=c("AAMRC1","BBMRC1","DDMRC1"))

matchIndexes <- match(geneNames,rownames(dataMat))
dataMat <- as.data.frame(t(dataMat[matchIndexes,]))
plotdata_sc_value <- as.numeric(unlist(lapply(by(dataMat,sampInfo,colMeans),norm_cross_col)))
plotdata_sc_value <- c(M1M2_plotdata_value,plotdata_sc_value)
plotdata_sc_clulabels <- rep(c("AAMRC1","BBMRC1","DDMRC1"),each=ncol(dataMat))
plotdata_sc_clulabels <- c(M1M2_plotdata_clulabels,plotdata_sc_clulabels)
plotdata_sc_genelabels <- rep(geneNames,3)
plotdata_sc_genelabels <- c(M1M2_plotdata_genelabels,plotdata_sc_genelabels)
plotdata_sc <- as.data.frame(cbind(plotdata_sc_clulabels,plotdata_sc_genelabels,plotdata_sc_value))
plotdata_sc$plotdata_sc_genelabels <- factor(plotdata_sc_genelabels,levels=geneNames)
plotdata_sc$plotdata_sc_clulabels <- factor(plotdata_sc_clulabels,levels=c("HM_M1","IPS_M1_rep1","IPS_M1_rep2","IPS_M1_rep3","IPS_M1_rep4","IPS_M1_rep5","AAMRC1","BBMRC1","DDMRC1","HM_M2","IPS_M2_rep1","IPS_M2_rep2","IPS_M2_rep3","IPS_M2_rep4","IPS_M2_rep5"))
plotdata_sc$plotdata_sc_value <- as.numeric(plotdata_sc_value)
...
pdf(file="/public/ZhangJinLab/project_erv/Dpan/IPSAnaly/M1M2_traditional_cluster_CD206_WithoutNon.pdf",width=10,height=10)
pheatmap(plotdata_sc,scale="column",cluster_rows=FALSE,cluster_cols=TRUE,color = colorRampPalette(c("green","white","red"))(50),fontsize=4)
dev.off()

pdf(file="/public/ZhangJinLab/project_erv/Dpan/IPSAnaly/M1M2_traditional_nocluster_CD206_WithoutNon.pdf",width=10,height=10)
pheatmap(plotdata_sc,scale="column",cluster_rows=FALSE,cluster_cols=FALSE,color = colorRampPalette(c("green","white","red"))(50),fontsize=4)
dev.off()