...
mydata_total_dir <- "/media/data/AA/outs/filtered_feature_bc_matrix/"
...
save(mydata_total, file="/media/data/AA/outs/filtered_feature_bc_matrix/10X_LAH_InHouse.RData")

pdf("/media/data/AA/outs/filtered_feature_bc_matrix/Scatter_UMAP_dim1VSdim2_InHouse.pdf",width=10,height=10)
DimPlot(mydata_total, reduction = 'umap', pt.size = 0.5)
dev.off()

pdf("/media/data/AA/outs/filtered_feature_bc_matrix/Scatter_tSNE_dim1VSdim2_InHouse.pdf",width=10,height=10)
...
for (genename in var.genes){
	...
	ggsave(paste("/media/data/AA/outs/filtered_feature_bc_matrix/",genename,".pdf",sep=""),width=5,height=5)
}
