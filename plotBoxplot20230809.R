...
# Part 1
EnsembIDs <- read.table(file="/public/ZhangJinLab/backdata/lahdata/M1statgenes.txt",sep="\t",header=FALSE,stringsAsFactors=FALSE)
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		pdf(paste("/public/ZhangJinLab/backdata/lahdata/M1statgenes/",genename,"_boxplot.pdf",sep=""),width=8,height=10)
...
write.table(res,file="/public/ZhangJinLab/backdata/lahdata/M1statgenes/M1statgenes_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		if(length(AA) > 3 && length(BB) > 3 && length(DD) > 3){
			...
			pdf(paste("/public/ZhangJinLab/backdata/lahdata/M1statgenes/",genename,"_CD80_boxplot.pdf",sep=""),width=8,height=10)
			p <- ggplot(data=exp_des, aes(x=Label,y=value)) + geom_quasirandom(aes(color=Label),method="frowney",width=0.3,size=1) + scale_color_manual(values=q4[c(1,2,4)]) + geom_boxplot(width=0.05,outlier.shape=NA,color="grey",fill="white",linetype=2) + stat_summary(fun.y=mean, geom="point", shape=20, size=10,  alpha=0.7, color="#8B814C")
			p <- p + xlab("") + ylab("Normalized expression level") + ggtitle("") + theme_classic()
			p <- p + theme(panel.grid.major=element_blank(), panel.grid.minor = element_blank(),panel.background=element_blank(), rect=element_rect(linetype=1), axis.line=element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11),legend.position="none") + theme(axis.text.x=element_text(vjust = 0.6, angle = 45))
			print(p)
...
write.table(res,file="/public/ZhangJinLab/backdata/lahdata/M1statgenes/M1statgenes_CD80_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)


EnsembIDs <- read.table(file="/public/ZhangJinLab/backdata/lahdata/M2statgenes.txt",sep="\t",header=FALSE,stringsAsFactors=FALSE)
mapInfo <- read.table(file="/public/ZhangJinLab/backdata/lahdata/AA/features.tsv.gz",sep="\t",header=FALSE,stringsAsFactors=FALSE)
matchIndexes <- match(EnsembIDs[,2],mapInfo[,1])
geneVec <- mapInfo[matchIndexes[which(!is.na(matchIndexes))],2]
EnsembIDs <- EnsembIDs[which(!is.na(matchIndexes)),1]
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		pdf(paste("/public/ZhangJinLab/backdata/lahdata/M2statgenes/",genename,"_boxplot.pdf",sep=""),width=8,height=10)
		p <- ggplot(data=exp_des,aes(x=Label,y=value)) + geom_quasirandom(aes(color=Label),method="frowney",width=0.3,size=1) + scale_color_manual(values=q4[c(1,2,4)]) + geom_boxplot(width=0.05,outlier.shape=NA,color="grey",fill="white",linetype=2) + stat_summary(fun.y=mean, geom="point", shape=20, size=10,  alpha=0.7, color="#8B814C")
		p <- p + xlab("") + ylab("Normalized expression level") + ggtitle("") + theme_classic()
		p <- p + theme(panel.grid.major=element_blank(), panel.grid.minor = element_blank(),panel.background=element_blank(), rect=element_rect(linetype=1), axis.line=element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11),legend.position="none") + theme(axis.text.x=element_text(vjust = 0.6, angle = 45))
		print(p)
...
write.table(res,file="/public/ZhangJinLab/backdata/lahdata/M2statgenes/M2statgenes_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		if(length(AA) > 3 && length(BB) > 3 && length(DD) > 3){
			...
			pdf(paste("/public/ZhangJinLab/backdata/lahdata/M2statgenes/",genename,"_CD80_boxplot.pdf",sep=""),width=8,height=10)
			p <- ggplot(data=exp_des, aes(x=Label,y=value)) + geom_quasirandom(aes(color=Label),method="frowney",width=0.3,size=1) + scale_color_manual(values=q4[c(1,2,4)]) + geom_boxplot(width=0.05,outlier.shape=NA,color="grey",fill="white",linetype=2) + stat_summary(fun.y=mean, geom="point", shape=20, size=10,  alpha=0.7, color="#8B814C")
			p <- p + xlab("") + ylab("Normalized expression level") + ggtitle("") + theme_classic()
			p <- p + theme(panel.grid.major=element_blank(), panel.grid.minor = element_blank(),panel.background=element_blank(), rect=element_rect(linetype=1), axis.line=element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11),legend.position="none") + theme(axis.text.x=element_text(vjust = 0.6, angle = 45))
			print(p)
...
write.table(res,file="/public/ZhangJinLab/backdata/lahdata/M2statgenes/M2statgenes_CD80_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
