...
EnsembIDs <- read.table(file="/media/data/AA/outs/filtered_feature_bc_matrix/HLAENIDs.txt",header=FALSE,stringsAsFactors=FALSE)
mapInfo <- read.table(file="/media/data/AA/outs/filtered_feature_bc_matrix/features.tsv.gz",header=FALSE,sep="\t",stringsAsFactors=FALSE)
matchIndexes <- match(EnsembIDs[,1],mapInfo[,1])
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		if(length(res) == 0){
			res <- c(EnsembIDs[iterer],(length(CXCL9AA)/ncol(normDataAA)),(length(CXCL9BB)/ncol(normDataBB)),(length(CXCL9DD)/ncol(normDataDD)),a$p.value,b$p.value,c$p.value)
		}else{
			res <- rbind(res,c(EnsembIDs[iterer],(length(CXCL9AA)/ncol(normDataAA)),(length(CXCL9BB)/ncol(normDataBB)),(length(CXCL9DD)/ncol(normDataDD)),a$p.value,b$p.value,c$p.value))
		}
		...
		pdf(paste("/media/data/AA/",EnsembIDs[iterer],"_boxplot.pdf",sep=""),width=8,height=10)
...
write.table(res,file="/media/data/AA/HLA_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
...
for (genename in geneVec){
	if(length(which(rownames(normDataAA)==genename)) == 1){
		...
		if(length(CXCL9AA) > 3 && length(CXCL9BB) > 3 && length(CXCL9DD) > 3){
			...
			pdf(paste("/media/data/AA/",EnsembIDs[iterer],"_CD80_boxplot.pdf",sep=""),width=8,height=10)
			p <- ggplot(data=exp_des, aes(x=Label,y=value)) + geom_quasirandom(aes(color=Label),method="frowney",width=0.3,size=1) + scale_color_manual(values=q4[c(1,2,4)]) + geom_boxplot(width=0.05,outlier.shape=NA,color="grey",fill="white",linetype=2) + stat_summary(fun.y=mean, geom="point", shape=20, size=10,  alpha=0.7, color="#8B814C")
			p <- p + xlab("") + ylab("Normalized expression level") + ggtitle("") + theme_classic()
			p <- p + theme(panel.grid.major=element_blank(), panel.grid.minor = element_blank(),panel.background=element_blank(), rect=element_rect(linetype=1), axis.line=element_line(colour = "black"),axis.title.x=element_text(size=14), axis.title.y=element_text(size=14),axis.text.x=element_text(size=11),axis.text.y=element_text(size=11),legend.position="none") + theme(axis.text.x=element_text(vjust = 0.6, angle = 45))
			print(p)
...
write.table(res,file="/media/data/AA/HLA_CD80_stat.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
